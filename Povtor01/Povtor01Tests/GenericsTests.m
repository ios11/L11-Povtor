...
@interface MyGenericCollection<__covariant ObjectType> : NSObject

@property (nonatomic, readonly, copy) NSArray<ObjectType> *items;
...
@implementation MyGenericCollection {
...
- (void)testNoGenerics {
    MyGenericCollection *collection = [[MyGenericCollection alloc] init];
...
- (void)testWithGenerics {
    MyGenericCollection<NSNumber *> *collection = [[MyGenericCollection alloc] init];
    [collection addItem:@(84)];
//    [collection addItem:@[ @1, @2, @3 ]];
//    [collection addItem:@{ @"x" : @"y",
//                           @4   : @5 }];
    ...
    NSArray<NSNumber *> *items = collection.items;
    ...
    [items enumerateObjectsUsingBlock:^(NSNumber * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
        NSLog(@"%lu : %@", (unsigned long)idx, item);
    }];
}
